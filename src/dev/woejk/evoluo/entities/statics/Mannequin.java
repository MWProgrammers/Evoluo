package dev.woejk.evoluo.entities.statics;

import dev.woejk.evoluo.Handler;
import dev.woejk.evoluo.gfx.Assets;
import dev.woejk.evoluo.tiles.Tile;

import java.awt.*;

/**
 * Created by Woejk on 12/05/2017.
 */
public class Mannequin extends StaticEntity
{


    public Mannequin(Handler handler, float x, float y) {
        super(handler, x, y, Tile.TILEWIDTH, Tile.TILEHEIGHT);

        bounds.x = 3;
        bounds.y = (int) (height / 2f);
        bounds.width = width - 6;
        bounds.height = (int) (height - height / 2f);
    }

    @Override
    public void tick()
    {

    }

    @Override
    public void die()
    {

    }

    @Override
    public void render(Graphics g)
    {
        g.drawImage(Assets.mannequin, (int) (x - handler.getGameCamera().getxOffset()), (int) (y - handler.getGameCamera().getyOffset()), width, height, null);
    }
}
